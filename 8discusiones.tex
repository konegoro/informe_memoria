...
eferencias mutables hacia nodos padres e hijos, junto con los largos ciclos de vida asociados a estas referencias, generó importantes dificultades para cumplir simultáneamente con las garantías del 
...
\subsection{Funciones de parsing}
...
el proceso falle con un error de análisis.
...
Para probar el funcionamiento de este proyecto, primero es necesario clonar el repositorio \texttt{name-server-rust} desde GitHub, disponible 
\href{https://github.com/niclabs/nameserver-rust/}{acá}.
...
superando el máximo permitido de 63. Esta inconsistencia constituye un error de formato según el RFC 1035, por lo que el servidor responde con \texttt{FORMERR}.
...
Cuando el servidor recibe una consulta que incluye un registro OPT con el bit DO activado, interpreta que el cliente está solicitando datos DNSSEC.
...
las respuestas del servidor o incluirlas en los \textit{master files}.
...
\begin{itemize}
    ...
    \item Los tipos de RRsets presentes en cada dominio.
...
\begin{itemize}
    \item Los RRsets almacenados y sus owner names.
...
Como parte del proceso DNSSEC, se implementaron rutinas para construir los registros \texttt{DS} de las zonas hijas. Los casos de prueba imprimen los \texttt{DS} generados para:
...
La impresión de estos registros es fundamental, ya que deben copiarse manualmente e insertarse posteriormente en los \textit{master files} de las zonas superiores.  
...
Del caso 16 al caso 22 es posible ejecutar distintas autenticaciones criptográficas DNSSEC de manera local utilizando la función \texttt{dnssec\_authentification}.
Estos casos corresponden exactamente a los mismos escenarios analizados previamente en el Capítulo 7.
