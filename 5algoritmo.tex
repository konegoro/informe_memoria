...
\subsection{find\_zone\_in\_catalog}\label{sec:find-zone}
...
\subsection{add\_rrsigs\_for\_delegation}\label{sec:rrsigs-delegation}
...
El envío de los registros \texttt{DS} junto con una delegación está descrito en el RFC 4035~\cite{RFC4035}, sección 2.4. Aclarar también que esta función se ejecuta siempre independientemente 
de si la hubo o no un referral, ya que si no hay \texttt{DS}  simplemente se omite su adición. Este aspecto podría ser mejorado en el futuro para evitar llamadas innecesarias a la función.
...
(\texttt{RRSIG}), que permite a los resolvers validar criptográficamente que los datos no han sido alterados. La función solo añade los \texttt{RRSIG} correspondiente al tipo de consulta original, 
es decir, si la consulta fue por registros \texttt{A}, solo se añadirán los \texttt{RRSIG} que firman los registros \texttt{A}. 

El comportamiento descrito está definido en el RFC 4035~\cite{RFC4035}, sección 2.2.
...
a la sección \texttt{Authority} del mensaje DNS (\texttt{response}) cuando el servidor genera una respuesta
de tipo \texttt{NO\_DATA} o \texttt{NXDOMAIN} bajo DNSSEC.
...
\subsubsection{Caso 1: el dominio existe, pero el tipo de registro no (respuesta \texttt{NO\_DATA})}
Si el dominio consultado está presente en la base de datos, pero no tiene registros del tipo solicitado (\texttt{qtype}),
entonces se deben incluir los registros \texttt{NSEC} y \texttt{RRSIG(NSEC)} asociados a ese mismo dominio en la sección \texttt{Authority}.
...
inexistencia de nombres o tipos de registros está definido en el RFC 4035~\cite{RFC4035}, sección 3.1.3

A continuación se mostrarán respuestas del algoritmo para distintos casos de prueba.

\section{Resultados según cada caso de consulta}

A continuación se muestran ejemplos de respuestas generadas por el algoritmo de búsqueda para distintos casos de consulta. Cada ejemplo incluye los registros presentes en la base de datos, 
la consulta realizada y la respuesta generada en el formato del comando \texttt{dig}.

Aclarar que la sección \texttt{Additional} no se añadirá en los ejemplos a menos que contenga registros, para simplificar la visualización.

\subsection{Consulta con coincidencia exacta con nombre de dominio y tipo}
...
Al consultar por \textbf{blog.example.com} el algoritmo encuentra un registro de tipo \texttt{CNAME} y lo guarda en la sección \texttt{Answer}. Luego, 
ejecuta nuevamente el algoritmo utilizando el nombre de dominio especificado en el \texttt{CNAME}, logrando así encontrar el registro con la dirección IP correspondiente.\\
...
\subsection{Consulta que sale de la zona autoritativa : Delgation con Glue Records}
...
Al consultar por la IP (por el tipo \texttt{A}) de \textbf{b.example.com}, se encuentra un registro de tipo \texttt{NS}. Al comprobar que no existe ningún registro \texttt{SOA} en ese dominio 
(lo que indica que el servidor no es autoritativo en este caso), se debe guardar el registro \texttt{NS} en la sección \texttt{Authority}. Además, se verifica si 
existe un registro \texttt{glue} correspondiente y, de ser así, se añade en la sección \texttt{Additional}.\\

También se añaden los registros \texttt{DS} y sus firmas \texttt{RRSIG} correspondientes en la sección \texttt{Authority}, esto ya que se está delegando
a una subzona. Asimismo se añade el registro \texttt{NSEC} y su firma para confirmar la inexistencia del registro \texttt{A} solicitado a través del mapa de bits.\\
...
usará el \texttt{wildcard} y se guardará la respuesta en la sección \texttt{Answer}. Además se añaden los registros \texttt{NSEC} y \texttt{RRSIG(NSEC)} correspondientes en la sección 
\texttt{Authority} para probar la inexistencia del nombre consultado.\\
...
Acá no se muestra lo que hay en la base de datos, ya que el cliente consultará por la IP de un dominio que no existe (\texttt{unknown.example.com}) y no hay \texttt{wildcard} que aplique. 
Para lograr esta respuesta se borró temporalmente el \texttt{wildcard} de los datos.\\
...
\subsection{Consulta sin coincidencia en el tipo: NO\_DATA}
...
Al consultar por un dominio que sí existe, pero que no posee registros del tipo solicitado, por ejemplo, un \texttt{MX} para \textbf{www.example.com}, la respuesta correspondiente es 
\texttt{NO\_DATA}. En este caso, se incluyen en la sección \texttt{Authority} los registros \texttt{NSEC} y \texttt{RRSIG(NSEC)}, los cuales permiten demostrar criptográficamente la 
inexistencia del tipo consultado mediante el mapa de tipos contenido en el \texttt{NSEC}.\\
...
El codigo de respuesta es \texttt{NO\_ERROR} pero no hay registros en la sección \texttt{Answer}, siendo esto equivalente a un caso de \texttt{NO\_DATA}.
